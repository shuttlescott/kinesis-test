...
func (cq *CardQuery) QueryPlayer() *PlayerQuery {
...
func (cq *CardQuery) WithPlayer(opts ...func(*PlayerQuery)) *CardQuery {
	query := &PlayerQuery{config: cq.config}
...
func (cq *CardQuery) sqlAll(ctx context.Context) ([]*Card, error) {
	...
	if query := cq.withPlayer; query != nil {
		...
		for i := range nodes {
			if nodes[i].player_cards == nil {
				continue
			}
			fk := *nodes[i].player_cards
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "player_cards" returned %v`, n.ID)
			}
			for i := range nodes {
